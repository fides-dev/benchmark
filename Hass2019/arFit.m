...
elseif(ar.config.optimizer == 2)
    options = ar.config.optim;
    options.GradObj = 'on';
    options.GradConstr = 'on';
    
    options.Algorithm = 'trust-region-reflective';
    options.SubproblemAlgorithm = 'lbl-factorize';
...
elseif(ar.config.optimizer == 6)
    [pFit, ~, exitflag, output, lambda, jac] = ...
        ...
        [],ar.config.optim);% x l, u are transposed because TRESNEI.m uses column notation, opposed to row notation in D2D
